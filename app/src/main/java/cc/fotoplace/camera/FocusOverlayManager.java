...
public class FocusOverlayManager {
    private static final String TAG = "dyb_FocusManager";
    ...
    private boolean mForcedAutoFocus;//是否自动对焦
    ...
    public void setPreviewSize(int previewWidth, int previewHeight, int yOffset) {
        Log.d("dyb_focus_manager", "setPreviewSize (" + previewWidth + "," + previewHeight
                + ")---1");
    ...
    public void onShutterDown() {
        if (!mInitialized || !mForcedAutoFocus)
            return;
    ...
    public void doSnap() {
        Log.d("dyb_focus_manager", "doSnap");
        if (!mInitialized) {
            Log.d("dyb_focus_manager", "not Initialized");
            return;
        }
    ...
    public void onAutoFocus(boolean focused, boolean shutterButtonPressed) {
        ...
        if (mState == STATE_FOCUSING_SNAP_ON_FINISH) {
            Log.d("dyb", "onAutoFocus STATE_FOCUSING_SNAP_ON_FINISH focused = " + focused);
            ...
            Log.d("dyb", "onAutoFocus STATE_FOCUSING focused = " + focused);
    ...
    public void onAutoFocusMoving(boolean moving) {
        ...
        if (moving && !mPreviousMoving) {
            mUI.onFocusStarted(false);
        } else if (!moving) {
            mUI.onFocusSucceeded(true);
        }
        mHandler.removeMessages(RESET_TOUCH_FOCUS);
        mHandler.sendEmptyMessageDelayed(RESET_TOUCH_FOCUS, RESET_TOUCH_FOCUS_DELAY/2);
    ...
    private void initializeFocusAreas(int x, int y) {
        ...
        Log.d("dyb_focus_manager", "focus area:" + ((Area) mFocusArea.get(0)).rect.toShortString());
    }
    ...
    private void capture() {
        Log.d("dyb_focus_manager", "capture");
    ...
    private void calculateTapArea(int x, int y, float areaMultiple, Rect rect) {
        y -= mOffsetY;
        int focusWidth = Math.min(mPreviewWidth, mPreviewHeight) >> 2;
        int areaSize = (int) (focusWidth * areaMultiple );
    ...
    public void setForceAutoFocus(boolean isForced) {
        mForcedAutoFocus = isForced;
    }
}
